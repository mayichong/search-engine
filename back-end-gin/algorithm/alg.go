...
func Algorithm(result string) ([]string, []string, time.Duration) {
	...
	*/
	...
	jsonFile3, err := os.Open(pathname)
	// if we os.Open returns an error then handle it
	if err != nil {
		fmt.Println(err)
	}
	...
	byteValue2, _ := ioutil.ReadAll(jsonFile3)
	json.Unmarshal(byteValue2, &searchResult)

	//搜索结果如果有空格，变成slice

	var wordsSplit []string
	
	wordsSplit = strings.Fields(result)
	...
	// if len(result) >= 12 && !strings.Contains(result, " "){
	// 	resultWords = x.Cut(result, use_hmm)
	// 	wordsSplit = append(wordsSplit, resultWords[0])
	// }

	// fmt.Println(wordsSplit)

	for i,v := range wordsSplit{
		result = v
		fmt.Println(result)
	...
	for {
		...
		if match {
			...
			for value := range s {
				...
				searchID = append(searchID, s[value])
				//继续分词，为了相关度搜索
	...
	if count != 0 {
		...
		fmt.Println("啊啊啊啊啊啊啊啊啊")


	for i, v := range searchID {
		if v == " " {
			count = i
			fmt.Println(i)
			break
	...
	var right []string
		...
		for index1,value1 := range searchID {
			if index1 < count {
				left = append(left,value1)
			}else {
				right = append(right,value1)
			}
		}
		str := fmt.Sprintf("%v", intersect.Simple(left, right))
		str = str[1:]
		str = str[:len(str)-1]
		fmt.Println(str)
	
		please := strings.Split(str, " ")
	
		for _,value3 := range please{
			
			returnResult = append(returnResult, m[value3])
		}
